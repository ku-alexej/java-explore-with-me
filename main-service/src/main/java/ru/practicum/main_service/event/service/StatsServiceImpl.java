...
public class StatsServiceImpl implements StatsService {
    ...
    public void addHit(HttpServletRequest request) {
        log.info("MainService - addHit: request={}", request);

        statsClient.addHit(appName, request.getRequestURI(), request.getRemoteAddr(),
                LocalDateTime.parse(LocalDateTime.now().format(Constants.DATE_FORMATTER), Constants.DATE_FORMATTER));
    }

    @Override
    public List<ViewStats> getStats(LocalDateTime start, LocalDateTime end, List<String> uris, Boolean unique) {
        log.info("MainService - getStats: start={}, end={}, uris={}, unique={}", start, end, uris, unique);
    ...
    public Map<Long, Long> getViews(List<Event> events) {
        log.info("MainService - getViews: events={}", events);
